...
module.exports = {
  ...
  resetPasswordForget: async (req, res) => {
    const { resettoken , newPasswd } = req.body;

    try {
      const message = await User.resetPasswordForget(resettoken, newPasswd);
      return res.status(200).json({ message });
    } catch (err) {
      console.error('Error:', err);
      return res.status(500).json({ message: 'An error occurred' });
  ...
  forgotPassword: async (req, res) => {
    ...
    try {
      ...
      const message = await User.forgotPassword(email, token);
      const resetPasswordLink = "http://localhost:3000" + "/resetpassword/" + token;
      await User.sendEmail(email, resetPasswordLink);
      return res.status(200).json({ message });
    } catch (err) {
      console.error('Error:', err);
      return res.status(500).json({ message: 'An error occurred' });
